...
def return_response(event):
    ...
    try:
	    ...
	    elseif event["mark"] == False:
	        event2["status"] = "invalidated"
	    else:
	    	event2["status"] = "mark not specified"
    except Exception as e:
    	event2["status"] = "validation error"
    return event2
...
@router.post("/go/execute_func", tags=["go"])
...
def execute_go(item: ExecuteGoInputs):
    ...
    try:
        ...
        event = append_keyvalue(event,"status",str(e))
    return atv_return_response(event)
